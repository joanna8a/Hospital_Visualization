...
void setup() {
  ...
  locationTable = new Tabla("locations.tsv");
  nameTable = new Tabla("names.tsv");
  rowCount = locationTable.getRowCount();  
  dataTable = new Tabla("random.tsv");


  asignStates();

  TeffCrSt = new Tabla("TimelyAndEffectiveCareState.csv");
  FIPS = new Tabla ("FIPS.csv");
  xLims = new Tabla("xLimits.csv");
  yLims = new Tabla("yLimits.csv");
  xPosCaps = new Tabla("xCaptsPos.csv");
  yPosCaps = new Tabla("yCaptsPos.csv");
  ...
  grpMain.addControls(Montgomery);
...
void draw() {
  ...
  //text(posCapsSts.getInt(0,0),1000, 50);
  //text(xCaps[0], 1200, 50);
  //text(xPosCaps.getInt(0,0), 800, 50);
  //text(yPosCaps.getInt(0,0), 1000, 50);
  //text(TeffCrSt.getString(1, 4), 1000, 50);
  //text(TeffCrScrs[1],1000,50);

  drawStates();
  //Pintaba los puntos (los de prueba funciona, los generados se salen del mapa)
...
public void LoadArchives() {
  ...
  for (int i = 1; i< TeffCrSt.getRowCount(); i++) {
    TeffCrScrs = new int[TeffCrSt.getRowCount()];
    TeffCrScrs[i] = 0;
  }
  ...
  for (int i = 1; i< TeffCrSt.getRowCount(); i++) {
    ...
    text(TeffCrSt.getString(1, 4), 100, 50);
    if (TeffCrStVls.length()==7) {
      TeffCrStVls = TeffCrStVls.substring(2, 5);
      TeffCrScrs[i] = parseInt(TeffCrStVls);
      text(TeffCrScrs[1], 1000, 50);
...
public void createWindows(int i) {
  ...
  wdws[i] = GWindow.getWindow(this, nmsSts[i], 20, 100, 920, 580, JAVA2D);
  wdws[i].addData(new MyWinData());
  ((MyWinData)wdws[i].data).col = col;
  ((MyWinData)wdws[i].data).index = i;
  //((MyWinData)wdws[i].data).state = states[i];
